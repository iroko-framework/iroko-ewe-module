# Skin and Topical Use

**IRI:**  
https://iroko-framework.github.io/iroko-ewe-module/terms/medicinalUse/skin-topical-use

**Type:**  
skos:Concept

**Preferred label:**  
Skin and topical use

**In scheme:**  
https://iroko-framework.github.io/iroko-ewe-module/terms/medicinalUse

---

## Definition

Associated with external or surface-level applications to the body.

---

## Scope note

Includes references to topical, dermal, or surface application. Does not include formulation details.

---

## Version

Introduced in v0.2.0
